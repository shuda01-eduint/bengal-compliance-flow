-- Update run_batch_eod to prioritize eod_investor_balance baseline
...
BEGIN
  ...
  inserted AS (
    ...
    RETURNING investor_code, closing_balance, gross_buy, gross_sell, commission, deposits, withdrawals
  )
  SELECT 
    COUNT(*),
...
-- Update process_staged_trades to prioritize eod_investor_balance baseline
...
BEGIN
  ...
  -- 1. eod_investor_balance (previous day) - official imported baseline
  -- 2. eod_ledger_snapshots (latest before this date) - snapshot chain
  -- 3. investors.ledger_balance - master table baseline
  -- 4. 0 - default for new accounts
