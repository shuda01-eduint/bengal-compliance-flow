-- Fix run_batch_eod function to use correct table and column names
...
BEGIN
  ...
  -- Calculate gross buy and gross sell from trade_history
  SELECT 
  ...
  WHERE trade_date = p_eod_date
    AND (UPPER(status) = 'FILL' OR UPPER(status) = 'PF' OR status IS NULL);
