-- Update get_accounting_data function to only count FILL type trades
...
BEGIN
  ...
  trade_agg AS (
    ...
    WHERE th.client_code IS NOT NULL
      ...
      -- This excludes PF (partial fills) and empty fill_type
      AND UPPER(COALESCE(th.fill_type, '')) = 'FILL'
...
-- Update get_accounting_summary function to only count FILL type trades
