...
-- Fix: Change opening balance priority to prefer snapshots over baseline imports
-- The issue: eod_investor_balance may have stale imported values that don't reflect
-- the actual calculated closing balance from the previous day's EOD run.
-- Solution: Check eod_ledger_snapshots FIRST, then fall back to eod_investor_balance
...
BEGIN
  ...
  -- STEP 3: Create temp table for opening balances with CORRECTED priority chain
  -- FIX: Prioritize eod_ledger_snapshots (actual calculated closing) over 
  -- eod_investor_balance (which may have stale imported values)
  CREATE TEMP TABLE tmp_opening_balances ON COMMIT DROP AS
  SELECT 
    COALESCE(ls.investor_code, ob.investor_code, inv.investor_code) AS investor_code,
    -- CORRECTED Priority: 1. Snapshot (actual calculated), 2. Baseline import, 3. Master table
    COALESCE(ls.closing_balance, ob.closing_ledger_balance, inv.ledger_balance, 0) AS opening_balance,
...
COMMENT ON FUNCTION public.process_staged_trades(date) IS 
'EOD processing function with CORRECTED opening balance priority chain.
Priority: 1. eod_ledger_snapshots (actual calculated closing from previous run)
          2. eod_investor_balance (imported baseline, may be stale)
          3. investors.ledger_balance (master table default)
This ensures calculated EOD closings take precedence over imported baselines.';
