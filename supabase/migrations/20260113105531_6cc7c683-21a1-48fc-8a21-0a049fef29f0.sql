...
DROP FUNCTION IF EXISTS get_negative_balance_codes(text, text, text, integer, integer);
DROP FUNCTION IF EXISTS get_negative_balance_codes(date, date, text);

-- Create new simplified function that queries eod_ledger_snapshots
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(e.rm_email, 'N/A') AS rm_name,
