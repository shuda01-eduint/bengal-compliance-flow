-- Fix process_staged_trades: Provide all required non-null columns for eod_investor_balance insert
...
BEGIN
  ...
  v_next_date := p_trade_date + INTERVAL '1 day';
  ...
  SELECT COUNT(*), 
         COALESCE(SUM(CASE WHEN side IN ('B', 'BUY') THEN value ELSE 0 END), 0),
         COALESCE(SUM(CASE WHEN side IN ('S', 'SELL') THEN value ELSE 0 END), 0),
  ...
  -- Priority 1: eod_investor_balance (official baseline)
  -- Priority 2: eod_ledger_snapshots (previous day snapshot chain)
  ...
  SELECT 
    investor_code,
    SUM(CASE WHEN side IN ('B', 'BUY') THEN value ELSE 0 END) AS gross_buy,
    SUM(CASE WHEN side IN ('S', 'SELL') THEN value ELSE 0 END) AS gross_sell,
  ...
  SELECT 
    ...
    s.total_withdrawals,
    s.closing_balance,
    s.closing_balance, -- ledger_balance = closing_balance
  ...
  -- Calculate equity = market_value - abs(negative_balance) - cumulative_interest
  SELECT 
    COALESCE(SUM(
      COALESCE(mv.total_mv, 0) 
  ...
  -- Insert/Update eod_investor_balance for the next day with today's closing balance
  -- Provide all NOT NULL columns with their default values
  ...
  SELECT 
    v_next_date,
    s.investor_code,
    s.closing_balance,  -- Today's closing becomes tomorrow's opening
    s.closing_balance,  -- Initialize closing = opening (will be updated when next day runs)
    0,  -- matured_balance default
    0,  -- receivable_sales default
    0,  -- cheque_in_tran_hand default
    s.cumulative_interest,  -- Carry forward cumulative interest
    COALESCE(mv.total_mv, 0) - CASE WHEN s.closing_balance < 0 THEN ABS(s.closing_balance) ELSE 0 END - s.cumulative_interest,  -- equity
    NULLIF(s.rm_id, '')  -- rm_id (convert empty string to NULL)
  FROM tmp_snapshots s
  LEFT JOIN tmp_mv_agg mv ON s.investor_code = mv.investor_code
