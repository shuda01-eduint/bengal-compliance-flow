-- Fix run_batch_eod function: correct side values and commission calculation
  ...
  WHILE v_current_date <= p_end_date LOOP
    ...
    FROM (
      SELECT 
        ...
        COALESCE(
          -- Get opening balance from previous day's EOD snapshot or clients table
          (SELECT ledger_balance FROM eod_ledger_snapshots 
           WHERE investor_code = c.inv_code 
           AND eod_date = v_current_date - INTERVAL '1 day'
           LIMIT 1),
          -- Fall back to balances_raw for the most recent date before current
          (SELECT ledger_balance FROM balances_raw 
           WHERE investor_code = c.inv_code 
           AND as_of_date <= v_current_date::text
        ...
        + COALESCE(
          (SELECT SUM(amount) FROM deposits_withdrawals 
           WHERE investor_code = c.inv_code 
           AND transaction_date = v_current_date::text
        ...
        -- Add/subtract trades with commission
        + COALESCE(
          (SELECT SUM(
            CASE 
