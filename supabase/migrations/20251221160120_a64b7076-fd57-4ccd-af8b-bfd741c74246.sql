...
-- Update get_accounting_data to use transaction dates for trade filtering
CREATE OR REPLACE FUNCTION public.get_accounting_data(_search_term text DEFAULT NULL::text, _account_type_filter text DEFAULT NULL::text, _has_trades_filter text DEFAULT NULL::text, _from_trade_date text DEFAULT NULL::text, _to_trade_date text DEFAULT NULL::text, _from_tx_date text DEFAULT NULL::text, _to_tx_date text DEFAULT NULL::text, _page_size integer DEFAULT 50, _page_offset integer DEFAULT 0)
...
BEGIN
  ...
  -- Determine which EOD date to use: find the latest EOD snapshot on or before the start date
  ...
  -- If no EOD snapshot found, fall back to current date logic
  IF eod_date_to_use IS NULL THEN
    eod_date_to_use := CURRENT_DATE;
  END IF;

  -- Convert transaction dates (yyyy-MM-dd) to trade_history format (yyyyMMdd)
  -- Use transaction dates for trade filtering to align with accounting period
  IF _from_tx_date IS NOT NULL THEN
  ...
  -- Get EOD ledger balances from the determined date
  eod_ledgers AS (
    SELECT 
  ...
  -- Fallback to clients table for ledger balances (only for investors not in EOD snapshot)
  client_ledgers AS (
...
-- Update get_accounting_summary to use transaction dates for trade filtering
