-- Update auto_create_missing_investors to use balances_raw + investors for baseline enrichment
-- Instead of admin_balances table which is empty
...
BEGIN
  ...
  WITH codes AS (
    ...
    -- Use DISTINCT ON to get one row per investor (since balances_raw may have multiple rows per investor for different instruments)
    ...
    -- Get commission, interest, department, name from existing investors that may have been imported
    -- This handles cases where investor data was bulk-updated during admin balance import
    SELECT 
      inv.investor_code,
      inv.investor_name,
    ...
    SELECT 
      ...
      COALESCE(bb.rm_name, bi.rm_name) AS rm_name,
      bb.rm_id,
      bi.department,
      bi.account_type
