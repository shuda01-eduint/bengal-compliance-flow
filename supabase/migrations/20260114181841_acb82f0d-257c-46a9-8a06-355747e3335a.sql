-- Drop and recreate run_batch_eod with optimization to skip balances_raw when previous EOD exists
    ...
    IF v_has_prev THEN
      ...
      daily_trades AS (
        SELECT 
          ...
          COALESCE(SUM(CASE WHEN side = 'B' THEN value ELSE 0 END), 0) as buy_value,
          COALESCE(SUM(CASE WHEN side = 'S' THEN value ELSE 0 END), 0) as sell_value
