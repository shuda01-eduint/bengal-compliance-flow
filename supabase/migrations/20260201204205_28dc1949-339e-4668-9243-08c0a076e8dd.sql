-- Fix process_staged_trades: prevent duplicate investor rows and add error handling
...
BEGIN
  ...
  -- If no previous snapshots, use balances_raw as baseline
  ...
  -- ============================================================
  -- STEP 3: Aggregate trades for the day
  -- ============================================================
  ...
  SELECT
    investor_code,
    SUM(CASE WHEN type IN ('DEPOSIT', 'deposit', 'Deposit') THEN amount ELSE 0 END) AS deposits,
    SUM(CASE WHEN type IN ('WITHDRAWAL', 'withdrawal', 'Withdrawal') THEN ABS(amount) ELSE 0 END) AS withdrawals
  ...
  trade_position_changes AS (
    SELECT
      investor_code,
      instrument,
      SUM(CASE WHEN UPPER(side) = 'BUY' THEN qty ELSE -qty END) AS qty_change,
      SUM(CASE WHEN UPPER(side) = 'BUY' THEN qty * price ELSE 0 END) AS buy_cost
    FROM trade_file
    WHERE trade_date = p_trade_date
  ...
  -- ============================================================
  -- STEP 8: Build base investor records with all calculations
  -- ============================================================
  ...
  SELECT
    wc.*,
    -- Daily interest only if negative balance (margin usage)
    CASE 
    ...
    -- Equity = market value - liability - interest
  ...
  -- FIXED: Add DISTINCT ON as safeguard against any remaining duplicates
  -- ============================================================
  ...
  INSERT INTO eod_run_history (
    ...
    v_total_withdrawals,
    'completed',
    'Processed via process_staged_trades'
  );
...
-- FIXED: Add exception handler for graceful error reporting
