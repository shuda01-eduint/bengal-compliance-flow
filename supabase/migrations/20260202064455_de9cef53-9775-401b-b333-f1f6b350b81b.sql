-- Add performance indexes for EOD processing
CREATE INDEX IF NOT EXISTS idx_trade_file_trade_date ON trade_file(trade_date);
CREATE INDEX IF NOT EXISTS idx_trade_file_investor_date ON trade_file(investor_code, trade_date);
CREATE INDEX IF NOT EXISTS idx_cash_ledger_txn_date ON cash_ledger_txn(txn_date);
CREATE INDEX IF NOT EXISTS idx_cash_ledger_txn_investor_date ON cash_ledger_txn(investor_code, txn_date);
CREATE INDEX IF NOT EXISTS idx_eod_ledger_snapshots_date ON eod_ledger_snapshots(eod_date);
CREATE INDEX IF NOT EXISTS idx_eod_ledger_snapshots_investor_date ON eod_ledger_snapshots(investor_code, eod_date);
CREATE INDEX IF NOT EXISTS idx_eod_investor_balance_date ON eod_investor_balance(trade_date);
CREATE INDEX IF NOT EXISTS idx_eod_investor_balance_investor_date ON eod_investor_balance(investor_code, trade_date);
CREATE INDEX IF NOT EXISTS idx_eod_instrument_position_date ON eod_instrument_position(trade_date);
CREATE INDEX IF NOT EXISTS idx_investors_code ON investors(investor_code);
CREATE INDEX IF NOT EXISTS idx_employees_email ON employees(email);

-- Optimized process_staged_trades function with higher timeout and batched operations
...
BEGIN
  ...
  -- STEP 5: Aggregate trades (materialized CTE equivalent)
  CREATE TEMP TABLE tmp_trades ON COMMIT DROP AS
  SELECT 
    tf.investor_code,
    SUM(CASE WHEN UPPER(tf.side) IN ('B', 'BUY') THEN tf.value ELSE 0 END) AS gross_buy,
    SUM(CASE WHEN UPPER(tf.side) IN ('S', 'SELL') THEN tf.value ELSE 0 END) AS gross_sell,
