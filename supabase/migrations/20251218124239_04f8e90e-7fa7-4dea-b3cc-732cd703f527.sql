-- Update get_accounting_data with server-side filters
...
-- Update get_accounting_summary with server-side filters
...
BEGIN
  ...
  SELECT 
    ...
    ft.total_buy,
    ft.total_sell,
    (ft.total_buy + ft.total_sell) as total_trade_value
  FROM with_closing, filtered_trade_totals ft
  GROUP BY ft.total_buy, ft.total_sell;
END;
$function$;