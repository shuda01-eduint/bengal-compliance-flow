-- Fix get_accounting_data_v3 to support date range filtering for trades
...
CREATE OR REPLACE FUNCTION public.get_accounting_data_v3(
  _opening_date text,  -- This is the EOD snapshot date (day before start date)
  _tx_date text,       -- This is the end date of the range
...
DECLARE
  v_opening_date date;
  v_tx_date date;
  v_trade_start_date date;
BEGIN
  -- Cast text parameters to date
  v_opening_date := _opening_date::date;
  v_tx_date := _tx_date::date;
  -- Trade start date is the day AFTER the opening balance snapshot
  v_trade_start_date := v_opening_date + interval '1 day';

  RETURN QUERY
  WITH opening_balances AS (
    -- Use eod_ledger_snapshots for opening balances (snapshot from day before start)
    SELECT 
      els.investor_code,
      COALESCE(els.ledger_balance, 0) AS opening_balance
    FROM eod_ledger_snapshots els
    WHERE els.eod_date = v_opening_date
  ),
  trade_sums AS (
    -- Sum trades from start date through end date (inclusive)
    SELECT 
      th.client_code,
    ...
    WHERE th.trade_date::date >= v_trade_start_date 
      AND th.trade_date::date <= v_tx_date
    GROUP BY th.client_code
  ),
  deposit_sums AS (
    -- Sum deposits/withdrawals from start date through end date (inclusive)
    SELECT 
      dw.investor_code,
    ...
    WHERE dw.transaction_date::date >= v_trade_start_date 
      AND dw.transaction_date::date <= v_tx_date
  ...
  base_data AS (
    SELECT 
      c.inv_code,
      COALESCE(i.investor_name, ob_name.investor_name, 'Unknown') AS investor_name,
    ...
    LEFT JOIN eod_ledger_snapshots ob_name ON c.inv_code = ob_name.investor_code AND ob_name.eod_date = v_opening_date
