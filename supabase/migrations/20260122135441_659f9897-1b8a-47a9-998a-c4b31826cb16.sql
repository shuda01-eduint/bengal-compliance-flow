-- Fix date=text comparison in get_accounting_data_v3 (7-arg overload)
-- Root cause: balances_raw.as_of_date and deposits_withdrawals.transaction_date are DATE columns,
-- but previous revision compared them to TEXT.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(tr.has_trades, false) AS has_trades,
    COUNT(*) OVER()::bigint AS total_count
  FROM investor_base ib
  LEFT JOIN rm_assignments ra ON ra.investor_code = ib.investor_code
  LEFT JOIN public.employees e_master ON LOWER(e_master.employee_id) = LOWER(ib.rm_id)
  ...
  WHERE (
    COALESCE(NULLIF(LOWER(_has_activity_filter), ''), 'all') = 'all'
    OR (LOWER(_has_activity_filter) IN ('yes','with_trades') AND COALESCE(tr.has_trades, false) = true)
    OR (LOWER(_has_activity_filter) IN ('no','no_trades') AND COALESCE(tr.has_trades, false) = false)
  )
...
-- Security linter: set immutable search_path on existing functions that were missing it
ALTER FUNCTION public.delete_all_holdings() SET search_path = 'public';
ALTER FUNCTION public.get_deposit_withdrawal_counts(date) SET search_path = 'public';
