-- Fix run_batch_eod function: case sensitivity and commission calculation
  ...
  WHILE v_current_date <= v_end_date LOOP
    ...
    day_trades AS (
      SELECT 
        ...
        SUM(CASE WHEN UPPER(side) = 'BUY' THEN COALESCE(value, 0) ELSE 0 END) as total_buy,
        SUM(CASE WHEN UPPER(side) = 'SELL' THEN COALESCE(value, 0) ELSE 0 END) as total_sell,
        -- FIX: Commission is always a cost, sum it separately
    ...
    calculated_balances AS (
      SELECT 
        ...
        -- FIX: Correct formula: opening + deposits - withdrawals - buys + sells - commission
        o.opening_balance 
