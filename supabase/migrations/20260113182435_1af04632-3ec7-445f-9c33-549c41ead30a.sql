...
-- Recreate with proper text comparison for trade_date (stored as YYYYMMDD text)
...
BEGIN
  ...
  trade_sums AS (
    ...
    WHERE (
      -- Compare trade_date as TEXT (stored as YYYYMMDD format)
      (_from_trade_date IS NULL OR th.trade_date >= _from_trade_date)
  ...
  deposit_sums AS (
    ...
    WHERE (
      -- transaction_date is stored as proper date type
