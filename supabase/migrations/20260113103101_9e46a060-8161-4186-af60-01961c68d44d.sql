...
BEGIN
  ...
  WITH trade_sums AS (
    SELECT
      th.client_code,
      SUM(CASE WHEN UPPER(th.trade_type) = 'BUY' THEN COALESCE(th.total_value, 0) ELSE 0 END) AS buy_turnover,
      SUM(CASE WHEN UPPER(th.trade_type) = 'SELL' THEN COALESCE(th.total_value, 0) ELSE 0 END) AS sell_turnover,
      SUM(COALESCE(th.commission, 0)) AS brokerage
    FROM trade_history th
    WHERE (NULLIF(_from_trade_date, '') IS NULL OR th.trade_date >= NULLIF(_from_trade_date, '')::date)
      AND (NULLIF(_to_trade_date, '') IS NULL OR th.trade_date <= NULLIF(_to_trade_date, '')::date)
