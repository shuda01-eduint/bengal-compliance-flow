...
-- Fix get_accounting_data to respect user roles
-- Drop existing functions first
DROP FUNCTION IF EXISTS get_accounting_data(text, text, text, date, date, integer, integer);
DROP FUNCTION IF EXISTS get_accounting_data(text, text, text, date, date, integer, integer, text, text);
DROP FUNCTION IF EXISTS get_accounting_summary(text, text, date, date);
DROP FUNCTION IF EXISTS get_accounting_summary(text, text, date, date, text, text);

-- Recreate get_accounting_data with role-based filtering
...
BEGIN
  ...
  WITH accessible_clients AS (
    -- Get investor codes the user can access
    SELECT DISTINCT c.inv_code as investor_code
    FROM clients c
    WHERE 
      _is_admin = true
      OR LOWER(c.rm_email) = _user_email
...
-- Recreate get_accounting_summary with role-based filtering
