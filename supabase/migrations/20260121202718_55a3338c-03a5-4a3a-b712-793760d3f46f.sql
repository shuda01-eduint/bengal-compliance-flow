...
BEGIN
  ...
  -- Convert date to trade_history format (yyyyMMdd text)
  ...
  WITH all_investor_codes AS (
    ...
    UNION
    -- From trade_history for current date (trade_date is TEXT in yyyyMMdd format)
    SELECT UPPER(client_code) as inv_code FROM trade_history WHERE trade_date = v_trade_date_str AND client_code IS NOT NULL
    UNION
    -- From deposits_withdrawals for current date (transaction_date is DATE)
