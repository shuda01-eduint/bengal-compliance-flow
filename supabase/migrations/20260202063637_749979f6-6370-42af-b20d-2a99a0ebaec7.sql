-- Fix UPDATE statements to include WHERE clause (required by Supabase)
...
BEGIN
  ...
  SELECT
    ...
    -- Pre-calculate interest fields
    CASE 
      WHEN (COALESCE(ob.opening_balance, 0) 
        + COALESCE(ca.total_deposits, 0) 
        - COALESCE(ca.total_withdrawals, 0)
        + COALESCE(ta.gross_sell, 0)
        - COALESCE(ta.gross_buy, 0)
        - COALESCE(ta.total_commission, 0)) < 0 
      THEN ABS(COALESCE(ob.opening_balance, 0) 
        + COALESCE(ca.total_deposits, 0) 
        - COALESCE(ca.total_withdrawals, 0)
        + COALESCE(ta.gross_sell, 0)
        - COALESCE(ta.gross_buy, 0)
        - COALESCE(ta.total_commission, 0)) * (COALESCE(im.investor_interest_rate, 15) / 100) / 365
      ELSE 0 
    END AS daily_interest,
    COALESCE(ob.cumulative_interest, 0) + CASE 
      WHEN (COALESCE(ob.opening_balance, 0) 
        + COALESCE(ca.total_deposits, 0) 
        - COALESCE(ca.total_withdrawals, 0)
        + COALESCE(ta.gross_sell, 0)
  ...
  -- Add equity column and calculate it
  ALTER TABLE tmp_snapshots ADD COLUMN equity numeric DEFAULT 0;

  -- Update equity with WHERE clause (required by Supabase)
  ...
  SELECT
    ...
    closing_balance,
    closing_balance, -- ledger_balance same as closing
    daily_interest,
    cumulative_interest,
    total_market_value - total_portfolio_cost -- unrealized P&L
  FROM tmp_snapshots;
  ...
  INTO v_trade_count, v_investor_count
  FROM tmp_trade_agg;
  ...
  SELECT 
    COALESCE(SUM(deposit_count), 0),
    COALESCE(SUM(withdrawal_count), 0),
  ...
  SELECT COALESCE(SUM(total_market_value), 0) INTO v_total_market_value FROM tmp_snapshots;

  -- Margin and interest stats
  SELECT 
    COUNT(*),
  ...
  INTO v_daily_interest_total, v_cumulative_interest_total
  FROM tmp_snapshots;
  ...
  -- RM assignment stats
  SELECT COUNT(*) INTO v_with_rm_assigned FROM tmp_snapshots WHERE rm_id IS NOT NULL;
  SELECT COUNT(*) INTO v_with_department FROM tmp_snapshots WHERE department IS NOT NULL;

  -- ============================================================
  -- STEP 10: Insert run history record
  -- ============================================================
