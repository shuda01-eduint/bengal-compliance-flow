-- Add ledger_balance_snapshot column to eod_ledger_snapshots for verification
ALTER TABLE eod_ledger_snapshots 
ADD COLUMN IF NOT EXISTS ledger_balance_snapshot numeric DEFAULT 0;

-- Update run_batch_eod function to source deposits/withdrawals from trade_history
...
BEGIN
  ...
  -- Calculate previous business date (simple: day before)
  ...
  BEGIN
    ...
    -- Include: investors with trades, deposits/withdrawals, previous snapshots, or in master data
    universe AS (
      ...
      FROM (
        ...
        UNION
        -- Investors with previous day snapshots (chain continuity)
