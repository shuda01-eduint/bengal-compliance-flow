-- Fix commission calculation to derive from investor brokerage rate when trade_file.commission is 0 or null
...
BEGIN
  ...
  SELECT DISTINCT ON (i.investor_code)
    ...
    -- Normalize brokerage rate: >= 0.1 means percentage (divide by 100), < 0.1 means already decimal, NULL defaults to 0.004
    CASE 
