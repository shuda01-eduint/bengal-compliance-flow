...
BEGIN
  ...
  -- STEP 2: Get all investors with activity or prior balance
  ...
  -- STEP 3: Get investor metadata with brokerage rate
  CREATE TEMP TABLE tmp_investor_meta ON COMMIT DROP AS
  SELECT DISTINCT ON (i.investor_code)
    i.investor_code,
    i.investor_name,
    i.brokerage_commission,
    -- Normalize brokerage rate: >= 0.1 means percentage (divide by 100), < 0.1 means already decimal, NULL/0 defaults to 0.004
    CASE 
  ...
  -- STEP 5: Aggregate trades with calculated commission from investor brokerage rate
  ...
  SELECT 
    ...
    -- Use trade commission if present and > 0, otherwise calculate from brokerage rate
    SUM(
      CASE 
        WHEN COALESCE(tf.commission, 0) > 0 THEN tf.commission
        ELSE COALESCE(tf.qty * tf.price, 0) * COALESCE(im.brokerage_rate, 0.004)
      END
    ) AS total_commission,
  ...
  SELECT 
    ...
    CASE 
      WHEN (COALESCE(ob.opening_balance, 0) - COALESCE(t.gross_buy, 0) + COALESCE(t.gross_sell, 0) - COALESCE(t.total_commission, 0) + COALESCE(cf.total_deposits, 0) - COALESCE(cf.total_withdrawals, 0)) < 0 
      THEN ABS(COALESCE(ob.opening_balance, 0) - COALESCE(t.gross_buy, 0) + COALESCE(t.gross_sell, 0) - COALESCE(t.total_commission, 0) + COALESCE(cf.total_deposits, 0) - COALESCE(cf.total_withdrawals, 0)) * COALESCE(im.interest_rate, 12) / 100 / 365
      ELSE 0
  ...
  -- NOTE: We intentionally DO NOT delete staging rows (trade_file/cash_ledger_txn).
  -- This allows re-running EOD for the same date without requiring a re-import.
