-- Fix date format mismatch: trade_date is YYYYMMDD but input is YYYY-MM-DD
-- Use REPLACE to normalize hyphenated dates to YYYYMMDD format
...
BEGIN
  ...
  trade_sums AS (
    ...
    WHERE (
      -- FIX: Convert hyphenated dates (YYYY-MM-DD) to YYYYMMDD format for proper comparison
