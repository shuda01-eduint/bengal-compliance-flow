...
DROP FUNCTION IF EXISTS get_accounting_data(text,text,text,text,text);
...
BEGIN
  ...
  WITH trade_sums AS (
    SELECT
      th.client_code,
      SUM(CASE WHEN UPPER(th.side) = 'BUY' THEN COALESCE(th.value, 0) ELSE 0 END) AS buy_turnover,
      SUM(CASE WHEN UPPER(th.side) = 'SELL' THEN COALESCE(th.value, 0) ELSE 0 END) AS sell_turnover,
