-- Update get_accounting_data function to allow admins to see ALL investors
CREATE OR REPLACE FUNCTION public.get_accounting_data(
  ...
  _has_trades_filter text DEFAULT 'all'::text
)
RETURNS TABLE(
  investor_code text,
  investor_name text,
...
BEGIN
  ...
  WITH accessible_clients AS (
    -- Admins see ALL investors directly from investors table
    SELECT i.investor_code
    FROM investors i
    WHERE _is_admin = true
    UNION
    -- Non-admins see only clients they have access to
...
-- Update get_accounting_summary function to allow admins to see ALL investors
