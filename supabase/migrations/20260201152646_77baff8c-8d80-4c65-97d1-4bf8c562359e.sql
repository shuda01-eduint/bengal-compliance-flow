-- Fix run_batch_eod: created_by uuid mismatch + withdrawal type logic + commission calculation
...
BEGIN
  ...
  today_trades AS MATERIALIZED (
    SELECT
      tf.investor_code,
      SUM(CASE WHEN UPPER(tf.side) = 'B' THEN tf.qty * tf.price ELSE 0 END) as gross_buy,
      SUM(CASE WHEN UPPER(tf.side) = 'S' THEN tf.qty * tf.price ELSE 0 END) as gross_sell,
      SUM(
        (tf.qty * tf.price) * 
        CASE 
  ...
  inserted AS (
    ...
    SELECT
      ...
      wi.closing_balance,
      v_user_id  -- Use UUID, not email text
...
-- Fix process_staged_trades: withdrawal type logic (accept both WITHDRAW and WITHDRAWAL)
...
BEGIN
  ...
  -- Aggregate trades with commission from investor rate
...
-- One-time data normalization: standardize type values
UPDATE cash_ledger_txn
SET type = 'WITHDRAW'
WHERE UPPER(TRIM(type)) = 'WITHDRAWAL';
