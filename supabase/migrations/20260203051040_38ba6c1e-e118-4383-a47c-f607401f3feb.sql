-- Update get_negative_balance_codes to use investors table for status filtering
-- The investors table has proper status values: Active, Closed, Suspended, Auto-Created
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(e.investor_name, i.investor_name, c.investor_name, '') as client_name,
    COALESCE(e.rm_name, i.rm_name, c.rm_name, '') as rm_name,
    e.ledger_balance as closing_balance
  FROM eod_ledger_snapshots e
  LEFT JOIN investors i ON i.investor_code = e.investor_code
  ...
  WHERE e.ledger_balance < 0
    ...
    -- Exclude closed accounts using investors table which has proper status values
    ...
    AND (
      ...
      OR COALESCE(e.investor_name, i.investor_name, c.investor_name, '') ILIKE '%' || p_search || '%'
      OR COALESCE(e.rm_name, i.rm_name, c.rm_name, '') ILIKE '%' || p_search || '%'
    )
