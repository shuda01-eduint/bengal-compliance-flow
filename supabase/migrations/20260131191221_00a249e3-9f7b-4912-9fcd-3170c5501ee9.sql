...
-- Fix: position_mv CTE scope issue - use temp table approach
-- Also fix search_path security warning
...
BEGIN
  ...
  -- Populate position market values
  ...
  SELECT
    ...
    CASE WHEN pa.net_qty > 0 AND pa.buy_qty > 0 THEN pa.buy_cost / pa.buy_qty ELSE 0 END as avg_cost,
  ...
  investor_snapshot AS (
    SELECT
      ...
      inv.account_type,
      COALESCE(imv.total_mv, 0) as total_mv
    FROM investors inv
